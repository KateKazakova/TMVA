    ...
    if(_catename=="Run1_8TeV"){
      ...
      if(m_channel==2 && _category>=0) _category=_category+3;
	    ...
	    SetMVAVars(postfix);
            SetMVAVars1(postfix);
            SetMVAVars2(postfix);
      double MLPOUT = GetMVAResponse1();
      double BDTGOUT = GetMVAResponse2();

      if(Map_float["llg_pTt"+postfix]>40 && Map_float["VBF_N_j"] == 0) _category = 1;
      else if(Map_float["llg_pTt"+postfix]<40 && Map_float["VBF_N_j"] == 0 && m_channel == 1) _category=2;
      else if(Map_float["llg_pTt"+postfix]<40 && Map_float["VBF_N_j"] == 0 && m_channel == 2) _category=3;

      else if(MLPOUT > 0.86 && Map_float["VBF_N_j"] == 1) _category=4;
      else if(MLPOUT < 0.86 && Map_float["VBF_N_j"] == 1) _category=5;

      else if(BDTGOUT > 0.74 && Map_float["VBF_N_j"] >=2 ) _category=6;
      else if(BDTGOUT < 0.74 && Map_float["VBF_N_j"] >=2 && m_channel == 1) _category=7;
      else if(BDTGOUT < 0.74 && Map_float["VBF_N_j"] >=2 && m_channel == 2) _category=8;
